...
export function buildLightroomControlledActivationV2(input = {}) {
  ...
  const gateChecks = [
    ...
    _gate({ name: 'human review completed', passed: flags.requireHumanReview ? false : true, required: flags.requireHumanReview, severity: 'critical', reason: flags.requireHumanReview ? 'No mechanism in this codebase can mark human review as complete — this gate can only be satisfied by an explicit future process outside this function.' : 'Human review is not required by the current flags, so this gate is not enforced.', source: 'Human Review Process' }),
  ...
  if (flags.requireHumanReview) blockers.push({ blocker: 'Human review is required before activation.', severity: 'critical', requiredFix: 'Complete a human review process (does not exist in this codebase yet) and record its result.', source: 'Human Review Process' });
