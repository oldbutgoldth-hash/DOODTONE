# EPIC 2E-K-R2-FIX5 Modified Files

## Added

- `qa/helpers/deterministic-indexeddb.mjs`
- `qa/epic-2e-k-r2-fix5-storage-contract-test.mjs`
- `qa/epic-2e-k-r2-fix5-storage-contract-results.json`
- `qa/epic-2e-k-r2-fix5-native-indexeddb-browser-test.mjs`
- `qa/epic-2e-k-r2-fix5-native-indexeddb-browser-results.json`
- `qa/epic-2e-k-r2-fix5-release-gate.mjs`
- `qa/epic-2e-k-r2-fix5-release-gate-results.json`
- `RUN_LUMIXA_FIX5_QA_WINDOWS.bat`
- `FIX5_QA_REPORT.md`
- `FIX5_RELEASE_NOTES.md`
- `FIX5_MODIFIED_FILES.md`
- `docs/project/40_EPIC_2E_K_R2_FIX5_STORAGE_RELEASE_GATE.md`

## Updated

- `package.json`
- `qa/run-static-suites.mjs`
- `qa/epic-2e-k-r2-fix2-save-gate-test.mjs`
- `README.md`
- `LUMIXA_PROJECT_MEMORY_R5.md`
- `docs/project/05_PROJECT_MEMORY.md`

## Explicitly unchanged

- Production Lightroom Mapping engine
- Preset engine
- XMP validator and Production XMP output
- `ui/app.js`
- `ui/ui-engine.js`
- FIX4 Preview-before-Candidate-Review behavior


## FIX5.1 Windows QA closure
- Corrected Windows Chrome/Edge paths with `path.join()`; previous template literals removed path separators.
- Added Chrome for Testing / Playwright cache discovery and PATH discovery.
- Native IndexedDB runner now passes Playwright Chromium into the unified detector.
- Browser contract static test now uses `process.execPath`, making it deterministic on Windows/macOS/Linux.
- `node_modules` is allowed in the QA workspace after `npm ci` and remains explicitly excluded from release ZIPs.
