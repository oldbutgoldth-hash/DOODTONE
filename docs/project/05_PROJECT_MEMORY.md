...
| EPIC 2D-F | Shadow Compare Honesty Patch — confirmed a real overclaiming bug: `safetyDelta.v2SaferThanLegacy` could return `true` even with `legacySummary.available=false`, a live hard stop, high/critical over-stack risk, or a low `globalSafetyScore` (verified concretely on the exact real-pipeline shape — `legacySummary.available=false` from `decision-engine` — before the patch: `v2SaferThanLegacy=true` with no `status` field at all). Rewrote `_buildSafetyDelta` with fully guarded logic and a new `status` field (`safer-estimate` / `similar` / `uncertain` / `riskier`) — `v2SaferThanLegacy` can now ONLY be `true` when legacy output is available, no hard stops exist, over-stack severity isn't high/critical, `globalSafetyScore` is adequate, and at least 2 concrete safety improvements are identified (verified both the failing real-pipeline case → `uncertain`/`false`, and a fully-healthy synthetic case with measured noise data → `safer-estimate`/`true`, proving the guard isn't simply always-false). A real design bug was also caught and fixed mid-patch: Safety Clamp's `activationGate.blockers[]` always carries 2 static, phase-level entries regardless of input quality, which would have made a "blockers exist" check permanently block any safer claim — fixed by counting only blockers beyond that baseline. `activationReadiness.level` now capped at `needs-more-shadow-data` (never `eligible-for-controlled-test`) whenever legacy is unavailable; `comparisonMatrix` no longer invents a legacy direction when unavailable (explicit check, not an implicit fallback); `expectedImprovement` hedges language ("potentially better") and lowers confidence when legacy is missing. Decision Report and Reference Transfer Report narration updated to reflect `status` honestly (never says "safer" unless `status==="safer-estimate"`). Bumped `core/project-version.js`/badges to "AI Workflow v1.0.2-f (EPIC 2D-F)". Verified: production XMP output unchanged, legacy `decision.styleBudget` unchanged, badge visible without scrolling (compacts to "AI v1.0.2-f") | `core/lightroom-mapping-engine/mapping-v2-shadow-compare.js` (main patch); small narration-language updates in `core/decision-report-engine/index.js` (automatic, since it reads the module's own now-honest summaries) and `core/reference-transfer-engine/index.js` (`stance` text rewritten to use the new `status` field); version bump in `core/project-version.js`/`index.html` |
