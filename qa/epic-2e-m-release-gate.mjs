#!/usr/bin/env node
/** EPIC 2E-M fail-closed release gate. */
...
const RESULT_PATH = path.join(__dirname, 'epic-2e-m-release-gate-results.json');
...
function run(label, rel) {
  ...
  return { label, rel, exitCode, status, stdoutTail: (result.stdout || '').slice(-6000), stderrTail: (result.stderr || '').slice(-3000) };
}
async function hashFile(rel) { return createHash('sha256').update(await fs.readFile(path.join(ROOT, rel))).digest('hex'); }
async function verifyProduction() {
  const manifest = JSON.parse(await fs.readFile(path.join(ROOT, 'qa/baselines/epic-2e-m-production-invariant.json'), 'utf8'));
  const mismatches = [];
...
async function sourceHash() {
  ...
  for (const rel of [
    'core/calibration-lab/cohort-save-feedback.js',
    ...
    'qa/epic-2e-m-guided-cohort-intake-browser-test.mjs',
...
const steps = [
  ...
  run('Guided Cohort Browser', 'qa/epic-2e-m-guided-cohort-intake-browser-test.mjs'),
];
const productionInvariant = await verifyProduction();
steps.push({ label: 'Production/XMP Source Invariant', rel: 'qa/baselines/epic-2e-m-production-invariant.json', exitCode: productionInvariant.status === 'PASS' ? 0 : 1, status: productionInvariant.status, stdoutTail: JSON.stringify(productionInvariant), stderrTail: '' });
...
const result = {
  epic: '2E-M', suite: 'GUIDED_COHORT_INTAKE_RELEASE_GATE', decision, completed: true,
  runId, startedAt, completedAt: new Date().toISOString(), sourceHash: await sourceHash(), steps, productionInvariant,
  boundary: 'Guided Cohort intake only. Controlled V2 remains Candidate/Preview-only; Production Mapping and XMP remain Legacy.',
};
await fs.writeFile(RESULT_PATH, JSON.stringify(result, null, 2) + '\n');
console.log(`\nEPIC 2E-M RELEASE DECISION: ${decision}`);
console.log('Evidence: qa/epic-2e-m-release-gate-results.json');
